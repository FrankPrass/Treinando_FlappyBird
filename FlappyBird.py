...
# True a IA joga False jogo manual
ai_jogando = True
geracao = 0
...
class Passaro:
    ...
    # define mascara para avaliar colisao
    def get_mask(self):
...
class Cano:
    ...
    def colidir(self, passaro):
        ...
        topo_ponto_de_colisao = passaro_mask.overlap(topo_mask, distancia_topo)
        base_ponto_de_colisao = passaro_mask.overlap(base_mask, distancia_base)

        if topo_ponto_de_colisao or base_ponto_de_colisao:
            return True
        else:
            return False
...
def main(genomas, config):  # fitness function
    ...
    if ai_jogando:
        ...
        passaros = [Passaro(230, 350)]  # Chama a __init__ da classe Passaro
    chao = Chao(730)    # Chama a __init__ da classe Chao
    canos = [Cano(700)]    # Chama a __init__ da classe Cano
    tela = pygame.display.set_mode((TELA_LARGURA, TELA_ALTURA))    # Cria a tela
    pontos = 0
    relogio = pygame.time.Clock()   # tempo de atualização da tela

    rodando = True
    while rodando:
        relogio.tick(30)    #30 fps

        # Iteração do jogador com o jogo, teclas
            ...
            if not ai_jogando:
                if evento.type == pygame.KEYDOWN:
                    if evento.key == pygame.K_SPACE or pygame.K_UP:
                        for passaro in passaros:
                            passaro.pular()
        # Definindo como IA olha para o cano da frente
        indice_cano = 0
        if len(passaros) > 0:
